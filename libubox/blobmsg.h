 ...
 *
 * 功能描述：blobmsg.h是blob.h的上层封装
 * 备注：提供了table和array两种扩展而来的消息类型
 */
#ifndef __BLOBMSG_H
#define __BLOBMSG_H
...
// 这是实现对齐的一种典型算法,最终得到的值BLOBMSG_PADDING(len)按照2^BLOBMSG_ALIGN对齐
...
// 根据消息名长度计算得到整个消息头长度（包含消息头结构体、消息名长度、字符串结尾符），4字节对齐
...
static inline void *blobmsg_data(const struct blob_attr *attr)
{
    ...
    // 判断扩展标记位（扩展标记位为0的消息意味着没有消息名，为1的意味着存在消息名）
	if (blob_is_extended(attr))
		data += blobmsg_hdrlen(be16_to_cpu(hdr->namelen));  // 有消息名的消息就需要掠过整个消息头，才能获取到消息值
...
static inline int blobmsg_data_len(const struct blob_attr *attr)
{
    ...
    // 如果扩展标记位为0（即不存在消息名），end和start指向同一个地址
    // 如果扩展标记位为1（即存在消息名），end和start相差一个消息头的长度
	start = (uint8_t *) blob_data(attr);
	end = (uint8_t *) blobmsg_data(attr);

    // 消息值长度计算不包含消息头（假如存在消息名）
	return blob_len(attr) - (end - start);
}

// 获取消息长度（这里等同于获取消息值长度处理）
