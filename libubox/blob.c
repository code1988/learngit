...
blob_buffer_grow(struct blob_buf *buf, int minlen)
{
	...
	buf->buflen += delta;                   // 调整后容量大小
	buf->buf = realloc(buf->buf, buf->buflen);  // 容量调整
	if (buf->buf)
...
blob_add(struct blob_buf *buf, struct blob_attr *pos, int id, int payload)
{
	...
	blob_init(attr, id, payload + sizeof(struct blob_attr));    // 为BLOB属性空间的id_len赋值
	blob_fill_pad(attr);                                        // 补充后的空间作清零操作(如果有需要)
	return attr;
}
