...
struct eloop_sock {
	int sock;                   // 描述符
	void *eloop_data;           // 回调函数参数2
	void *user_data;            // 回调函数参数3
	eloop_sock_handler handler; // 事件发生时的回调函数
	WPA_TRACE_REF(eloop);
	WPA_TRACE_REF(user);
	WPA_TRACE_INFO
};
...
struct eloop_sock_table {
	int count;                  // 已经注册的socket个数
	struct eloop_sock *table;   // 列表
	int changed;
};
