...
{/* 17.19.8 */
  ...
  {
    port->msgPortRole = (port->msgFlags & PORT_ROLE_MASK) >> PORT_ROLE_OFFS;    // 获取端口角色
#ifdef RSTP_JWS
    ...
    // 如果该bpdu的端口角色是指定端口 || 该bpdu是配置类型消息
    if (RSTP_PORT_ROLE_DESGN == port->msgPortRole || BPDU_CONFIG_TYPE == port->msgBpduType) 
    {
        // 比较消息优先级向量和端口优先级向量
    ...
    // 如果该bpdu的端口角色是指定端口 || 该bpdu是配置类型消息(同上)
    if (BPDU_CONFIG_TYPE == port->msgBpduType || RSTP_PORT_ROLE_DESGN == port->msgPortRole) 
    {
        // 比较消息优先级向量和端口优先级向量、消息时间和端口时间
...
{/* 17.19.13 */
  ...
  // 如果收到的bpdu中消息类型为BPDU_TOPO_CHANGE_TYPE
...
// 版本识别（stp 还是rstp）
// rstp 能够识别stp bpdu，stp不能识别rstp bpdu，所以同一拓扑中同时由两种设备时，协议将退化位stp
...
{/* 17.19.19 */
    ...
    {
        // 这种情况下取 min(max age - 有效年龄，hellotime * 3)
...
// 根据接收到的bpdu报文中bpdu类型，维护port控制块的各成员变量的更新,提供状态机切换的触发事件
...
{  
  ...
  {
    case BPDU_CONFIG_TYPE:      // config bpdu报文
      port->rx_cfg_bpdu_cnt++;          // 相应报文数量累加
#if 0 /* def STP_DBG */
      if (port->info->debug) 
        stp_trace ("CfgBpdu on port %s", port->port_name);
#endif
      if (port->admin_non_stp) return;  // 检查生成树功能是否打开
      port->rcvdBpdu = True;            // 标记收到bpdu报文
      // config bpdu报文在这里只是退出switch流程
      break;
    case BPDU_TOPO_CHANGE_TYPE: // TCN bpdu报文
      port->rx_tcn_bpdu_cnt++;          // 相应报文数量累加
#if 0 /* def STP_DBG */
      if (port->info->debug)
      ...
      if (port->admin_non_stp) return;  // 检查生成树功能是否打开
      port->rcvdBpdu = True;            // 标记收到bpdu报文
      port->msgBpduVersion = bpdu->hdr.version;  // 记录bpdu版本
      port->msgBpduType = bpdu->hdr.bpdu_type;   // 记录bpdu类型
      // TCN bpdu报文在这里就彻底结束了
      return;
    default:
      stp_trace ("RX undef bpdu type=%d", (int) bpdu->hdr.bpdu_type);
      return;
    case BPDU_RSTP:             // rstp bpdu报文
      port->rx_rstp_bpdu_cnt++;         // 相应报文数量累加
      if (port->admin_non_stp) return;  // 检查生成树功能是否打开
      // 只有当生成树版本大于等于NORMAL_RSTP时，才支持rstp bpdu报文,否则直接丢弃
...
#endif
      // rstp bpdu报文在这里只是退出switch流程
      break;
  }

  // 执行到这里意味着这是一个config/rstp bpdu报文
  port->msgBpduVersion = bpdu->hdr.version;      // 记录bpdu版本
  port->msgBpduType =    bpdu->hdr.bpdu_type;    // 记录bpdu类型
  port->msgFlags =       bpdu->body.flags;       // 记录bpdu主体中携带的flag标记，里面主要包含拓扑变化相关的信息
...
void STP_info_enter_state (STATE_MACH_T* this)
{
    ...
    switch (this->State) 
    {
        case BEGIN:     // 开始态，也就是非有效态，自定义，用于状态机启动初始化
