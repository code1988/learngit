...
struct command {
	...
	int optargs;        
	const char *name;   // 命令名
	int (*func) (int argc, char *const *argv);  // 命令对应的回调函数
	const char *help;   // 命令解释
};
...
static int cmd_exec(int argc, char** argv)
{
	...
	if ((cmd = cmd_lookup(argv[0])) == NULL) {
		syslog(LOG_ERR, "%s:Not found command '%s'\n", __func__,argv[0]);
		return 1;
	}
...
int main(int argc, char** argv)
{
	...
	int conf_input = 0;
	const struct command *cmd;  // 指向命令列表中的命令
	int f;
    ...
    // 命令查找
	if ((cmd = cmd_lookup(argv[0])) == NULL) {
		fprintf(stderr, "%s:Not found command '%s'\n", __func__,argv[0]);
		return 1;
	}
