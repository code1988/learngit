 ...
 * 运行时环境的安全检查,实际就是检查适配的libevent版本是否有效
...
int main (int argc, char **argv) {
    ...
    // 首先进行运行时环境的安全检查,实际就是检查适配的libevent版本是否有效
    if (!sanitycheck()) {
        return EX_OSERR;
    }
    ...
    while (-1 != (c = getopt(argc, argv, shortopts))) {
        ...
        case 'b' :
            ...
            break;
        case 'B':       // 配置绑定的协议名(通常用于sasl功能),支持3种协议:auto,binary,ascii
        ...
        case 'I':
            ...
            free(buf);
            break;
        case 'S': /* set Sasl authentication to true. Default is false  配置sasl(简单认证和安全层)功能,缺省不开 */
#ifndef ENABLE_SASL
    ...
    // 如果配置了开启sasl功能,则在这里配置要绑定的协议,缺省使用binary
    if (settings.sasl) {
    ...
    // 如果配置了UDP端口却没有配置TCP端口,则TCP端口默认同步为UDP端口
    if (udp_specified && settings.udpport != 0 && !tcp_specified) {
        settings.port = settings.udpport;
    }

    // 如果配置了限制core文件大小,则在这里进行限制
    if (maxcore != 0) {
     ...
     * 如果进程的初始权限为root权限,则需要切换到一个普通用户权限
     * */
