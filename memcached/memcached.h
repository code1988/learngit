...
enum protocol {
    ...
    binary_prot,
    negotiating_prot /* Discovering the protocol  自动协商 */
};
...
struct settings {
    ...
    int chunk_size;
    int num_threads;        /* number of worker (without dispatcher) libevent threads to run 工作线程数量(0<n<=64) */
    ...
    int reqs_per_event;     /* Maximum number of io to process on each
                               io-event. */
    bool use_cas;
    enum protocol binding_protocol; // sasl功能关联的协议类型
    int backlog;
    ...
    char *hash_algorithm;     /* Hash algorithm in use 记录了程序使用的hash算法名 */
...
typedef struct {
    ...
    struct conn_queue *new_conn_queue; /* queue of new connections to handle 指向该线程等待处理的新连接队列 */
...
struct conn {
    ...
    struct iovec *iov;      // iovec池
    int    iovsize;   /* number of elements allocated in iov[]  记录了iovec池中的iovec总数 */
    int    iovused;   /* number of elements used in iov[]  记录了已经使用的iovec数量 */

    struct msghdr *msglist; // msghdr池
    int    msgsize;   /* number of elements allocated in msglist[]  记录了msghdr池中的msghdr总数 */
    int    msgused;   /* number of elements used in msglist[]  记录了已经使用的msghdr数量 */
    ...
    /* Binary protocol stuff */
    /* This is where the binary header goes  记录了当前收到二进制报文的头部信息 
     * 备注: 该二进制头跟实际收到的报文中二进制头存在区别: 本地字节序
     * */
    protocol_binary_request_header binary_header;
    uint64_t cas; /* the cas to return */
    short cmd; /* current command being processed  记录了当前需要处理的命令 */
    int opaque;
    int keylen;
