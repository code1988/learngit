...
一个典型的lwip系统包含3个进程：首先是上层应用程序进程，然后是lwip协议栈进程，最后是底层硬件数据包接收进程（目前的A8工程中，最后一个进程不存在，其功能被放入了底层中断函数中，合理性需要做确认）


一.	链路层
	该层最核心的结构体:	struct netif{
							struct netif *next;			// 指向下一个netif结构体指针,双网卡驱动这里就需要指向另一块网卡的netif结构体
							struct ip_addr ip_addr;		// IP
							struct ip_addr netmask;		// 子网掩码
							struct ip_addr gw;			// 网关
							err_t (*input)(struct pbuf *p,struct netif *inp);							// 调用input函数可以从网卡上取得一个数据包,提交给IP层
							err_t (*output)(struct netif *netif,struct pbuf *p,struct ip_addr *ipaddr);	// IP层调用output函数可以向网卡发送一个数据包
...
三.传输层（这里主要讲TCP）
		...
		******************************************************************************************************************************************************************************************************	
		...
		void tcp_receive(struct tcp_pcb *pcb)
		{
			...
			if (flags & TCP_ACK) 
			{
				right_wnd_edge = pcb->snd_wnd + pcb->snd_wl2;	// 获取本地发送窗口右边界
			...
			if (tcplen > 0) 
			{
				// 如果seqno <= rcv_nxt <= seqno + tcplen，意味着数据头包含重复数据，需要截断(对比注释，这里存在疑问)
