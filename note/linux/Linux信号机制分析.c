...
3. 信号在进程中注册
    ...
    信号通知给进程，实质就是将该信号标记到signal域中，并且所携带的信息被记录到sigqueue链表的节点中。
    当一个实时信号发送给进程时，不管该信号是否已经在进程的signal域中被标记，都会作为一个新的节点插入sigqueue链表，因此，信号不会丢失
    当一个非实时信号发送给进程时，如果该信号已经在进程的signal域中被标记，则该信号将被丢弃，造成信号丢失
