栈帧的实现实际跟cpu本身的架构密切相关，这里主要对i386和arm32两种架构下的栈帧展开分析。
...
2. arm32中的栈帧
相对于i386架构cpu上相对统一的栈帧结构，arm32架构cpu上的栈帧实现就显得非常多变，这里将分析的是较典型的一种。
下面是arm32架构cpu上跟栈帧相关的一组寄存器：
