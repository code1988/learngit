...
static int OnKeyDown(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	switch(wParam) {
		...
		case VK_F7:       //save
			...
			tcpfsn_upload_port_get(&serverIP,&serverPort);
			printf("old server ip:%x\n",serverIP);
			serverIP = ((atoi(keynum+9) & 0xFF) << 24) ;
			keynum[9] = '\0';
			serverIP |= ((atoi(keynum+6) & 0xFF) << 16);
			keynum[6] = '\0';
			serverIP |= ((atoi(keynum+3) & 0xFF) << 8);
			keynum[3] = '\0';
			serverIP |= (atoi(keynum) & 0xFF);
		...
		case VK_F8:    
			spi_keyalert();
			DestroyWindow(hWnd);
			//printf("serverIPover1 \n");
			myMaskwin_create(parasetwin_handle_get());
			//printf("serverIPover2\n ");
			break;
		default:	
			spi_keyalert();
			DestroyWindow(hWnd);
			SetFocus(GetParent(hWnd));	// return 
			break;
	}
	return 0;
}
