...
typedef struct tagMoneyInfoS {
	char			*picptr;
	char			*crown;
	unsigned int	piclen;
	unsigned int	crownlen;
	unsigned short	sequence;
	unsigned char	valuta;
	unsigned char	ver;
	unsigned char	fact;
	unsigned char	type;
}MONEYINFO_S;
...
status_t fotatcp_server_init(void);
status_t fotatcp_server_release(void);
status_t fotatcp_send_data(void_t* h, s8_t *ptr, s32_t size);
status_t fota_protocol_header(u8_t *buf, u32_t size, s8_t fc);
status_t fotatcp_keeplive_set(void);
...
status_t v1_adjustresponse(FOTA_ANALYZE_S *h, u8_t *ptr);
status_t v1_adjustconfirm(FOTA_ANALYZE_S *h, u8_t *ptr);


status_t v1_blackresponse(FOTA_ANALYZE_S *h, u8_t *ptr);
status_t v1_paramread_response(FOTA_ANALYZE_S *h, u8_t *ptr);
status_t v1_paramsave_response(FOTA_ANALYZE_S *h, u8_t *ptr);
