...
struct blob_buf b;  // 定义一个BLOB
static struct ubus_msg_buf *retmsg; // 定义一个指向ububs消息控制块的指针
static int *retmsg_data;            // 指向ubus消息控制块的二阶属性空间的数据指针
static struct avl_tree clients;     // 客户端avl树管理结构
...
// ubusd向新加入连接的客户端发送一个hello消息
static bool ubusd_send_hello(struct ubus_client *cl)
{
	struct ubus_msg_buf *ub;

	blob_buf_init(&b, 0);               // BLOB控制块初始化
	ub = ubus_msg_from_blob(true);      // 申请一个ubus消息控制块（使用外部属性空间）
	if (!ub)
		return false;

	ubus_msg_init(ub, UBUS_MSG_HELLO, 0, cl->id.id);    // 设置消息头
	ubus_msg_send(cl, ub, true);        // 向新加入连接的客户端发送消息
	return true;
}
...
struct ubus_client *ubusd_proto_new_client(int fd, uloop_fd_handler cb)
{
    ...
    // 初始化节点成员
	INIT_LIST_HEAD(&cl->objects);   // 初始化新节点的链表指针
	cl->sock.fd = fd;               // 注册客户端的fd
	cl->sock.cb = cb;               // 注册客户端的回调函数
	cl->pending_msg_fd = -1;
    ...
    // ubusd向新加入连接的客户的发送一个hello消息作为连接建立的标志
...
{
	ubus_init_id_tree(&clients);                // 初始化一棵avl树

	blob_buf_init(&b, 0);                       // 初始化BLOB模块
	blob_put_int32(&b, UBUS_ATTR_STATUS, 0);    // 设置BLOB数据属性

	retmsg = ubus_msg_from_blob(false);         // 申请一个ubus消息控制块(使用内部属性空间)
	if (!retmsg)
		exit(1);

	retmsg->hdr.type = UBUS_MSG_STATUS;         // 设置ubus消息类型为 通用消息命令
	retmsg_data = blob_data(blob_data(retmsg->data));// 返回ubus消息控制块的二阶属性空间的数据指针
}
