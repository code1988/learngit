...
// ubus消息控制块的BLOB属性空间类型检测，确保都是使用内部BLOB属性空间
static struct ubus_msg_buf *ubus_msg_ref(struct ubus_msg_buf *ub)
{
    // 如果BLOB的属性空间为外部共享空间,则需要新申请一个带内部BLOB属性空间的ubus消息控制块
	if (ub->refcount == ~0)
...
// 申请一个ubus消息控制块（data：指向BOLB属性空间 len:属性空间全长（含头）share:属性共享标志）
struct ubus_msg_buf *ubus_msg_new(void *data, int len, bool shared)
{
	struct ubus_msg_buf *ub;
	int buflen = sizeof(*ub);

    // 判断属性共享标志，决定是否为属性空间申请内存
	if (!shared)
		buflen += len;

    // 分配空间（这里是ubus消息控制块大小+属性空间大小）
	ub = calloc(1, buflen);
	if (!ub)
		return NULL;
...
static int ubus_msg_writev(int fd, struct ubus_msg_buf *ub, int offset)
{
    ...
    {
		iov[0].iov_base = ((char *) &ub->hdr) + offset; // 指向消息头
		iov[0].iov_len = sizeof(ub->hdr) - offset;      // 记录消息头长度
		iov[1].iov_base = (char *) ub->data;            // 指向消息具体内容
		iov[1].iov_len = ub->len;                       // 记录消息内容长度
...
void ubus_msg_send(struct ubus_client *cl, struct ubus_msg_buf *ub, bool free)
{
	...
	if (!cl->tx_queue[cl->txq_cur]) 
    {
        ...
        // 函数运行到这里意味着本条消息没有全部发送完毕
		if (written < 0)
			written = 0;

		cl->txq_ofs = written;  // 记录已经发送的消息长度

		/* get an event once we can write to the socket again */
        // 由于没有全部发送完毕，这里将客户端继续注册到epoll中时，需要添加可写事件
		uloop_fd_add(&cl->sock, ULOOP_READ | ULOOP_WRITE | ULOOP_EDGE_TRIGGER);
	}

    // 函数运行到这里意味着2种可能：客户端控制块上的消息池不空;新的消息没有全部发送完毕
...
int main(int argc, char **argv)
{
	...
	signal(SIGPIPE, SIG_IGN);   // 防止ubusd服务器进程收到SIGPIPE信号后自动退出

	uloop_init();               // 创建非阻塞epoll句柄
    ...
    // 注册服务器监听套接字（边沿触发方式监听是否可读）到epoll
