 ...
 * 为每个接口以及所属成员（主要是bss）分配空间，并完成常量性质成员的初始化
 */
...
static int hostapd_driver_init(struct hostapd_iface *iface)
{
    ...
    // 完成此bss指定的驱动器的初始化，返回一个指定类型的驱动器的控制块
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		case 'h':
			usage();
			break;
		case 'd':   // 命令行中每出现一个d则debug级别递增，同时日志级别递减
			debug++;
    ...
    // 根据命令行配置文件数量初始化hapd_interfaces结构体
