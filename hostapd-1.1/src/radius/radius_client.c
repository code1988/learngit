...
struct radius_client_data {
	 ...
	 */
	void *ctx;  // 指向所属的bss，也就是hostapd_data

	/**
	 * conf - RADIUS client configuration (list of RADIUS servers to use)
	 */
	struct hostapd_radius_servers *conf;    // radius客户端配置信息
		       ...
		       const u8 *addr)
{
    ...
    // 如果是一个RADIUS_ACCT_INTERIM数据包，则需要删除等待列表中可能存在的相同类型数据包
	...
	hostapd_logger(radius->ctx, NULL, HOSTAPD_MODULE_RADIUS,
		       ...
		       "server", name);
    // 如果设置了msg_dumps标记，则在打印该radius报文
	if (conf->msg_dumps)
		radius_msg_dump(msg);
...
// radius报文接收回调函数(收到认证帧、计费帧都采用此回调)
...
static int radius_client_init_auth(struct radius_client_data *radius)
{
    ...
    // 为收到radius认证报文注册回调函数radius_client_receive
	if (radius->auth_serv_sock >= 0 &&
 ...
 * 每个bss上radius客户端控制块创建并初始化，最初的入口在hostapd_setup_interface函数中
 */
struct radius_client_data *
