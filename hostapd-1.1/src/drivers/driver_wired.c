...
struct wpa_driver_wired_data {
	char ifname[IFNAMSIZ + 1];  // 接口名
	void *ctx;                  // 指向父结构hostapd_data 

	int sock; /* raw packet socket for driver access */
	int dhcp_sock; /* socket for dhcp packets */
	int use_pae_group_addr;     // 标志位，是否使用多播地址
...
static void handle_data(void *ctx, unsigned char *buf, size_t len)
{
	...
	switch (ntohs(hdr->ethertype)) {
		case ETH_P_PAE:
			...
			wpa_supplicant_event(ctx, EVENT_NEW_STA, &event);   // 驱动层添加一个潜在的新mac 
...
// wire类型驱动器初始化socket接口
// 采用链路层套接字，以太网协议类型：0x888e,注册读事件函数handle_read,过滤多播地址01:80:C2:00:00:03
...
// 定义了wire类型驱动器API操作集合,供 ./drivers.c 中使用
