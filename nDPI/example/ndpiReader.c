...
/** Client parameters **/
static char *_pcap_file[MAX_NUM_READER_THREADS]; /**< Ingress pcap file/interfaces  记录了所有传入的pcap文件或捕获接口 */
static FILE *playlist_fp[MAX_NUM_READER_THREADS] = { NULL }; /**< Ingress playlist */
static FILE *results_file = NULL;       // 指向一个用于记录测试信息的文件句柄
static char *results_path = NULL;       // 指向一个用于记录测试信息的文件路径
static char *_bpf_filter      = NULL; /**< bpf filter  指向一个BPF过滤字串 */
static char *_protoFilePath   = NULL; /**< Protocol file path  指向一个协议文件路径 */
static char *_statsFilePath   = NULL; /**< Top stats file path */
#ifdef HAVE_JSON_C
static char *_jsonFilePath    = NULL; /**< JSON file path  指向一个JSON文件路径 */
#endif
#ifdef HAVE_JSON_C
...
static void parseOptions(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, "df:g:i:hp:l:s:tv:V:n:j:rp:w:q0123:456:7:89:m:b:", longopts, &option_idx)) != EOF) {
    ...
    case 'i':       // 指定要被nDPI处理的pcap文件(文件列表)或nDPI要嗅探的接口(接口列表)，列表的分隔符为","
    case '3':
      _pcap_file[0] = optarg;
      break;
    ...
    case 'n':       // 指定线程数量，缺省跟接口数量对应，(注：只有当指定了单个接口时该选项才生效)
  ...
  if(_pcap_file[0] == NULL || strcmp(_pcap_file[0], "") == 0) {
    ...
    if(strchr(_pcap_file[0], ',')) { /* multiple ingress interfaces */
        ...
        // 如果是单个接口/pcap文件，缺省就是1个线程(但可以通过命令行-n手动指定)
 ...
 * 强行终止libpcap库接口pcap_dispatch/pcap_loop的捕获操作
 */
...
int main(int argc, char **argv) {
  ...
  // aho-corasick自动机单元测试(头疼！暂时忽略这部分)
  automataUnitTest();
  ...
  // 使用了JSON格式输出或者处于安静模式下，不再打印到标准输出，下同
  if((!json_flag) && (!quiet_mode)) {
    printf("\n-----------------------------------------------------------\n"
  ...
  // 注册SIGINT信号处理函数，用于强行终止正在进行的捕获操作
