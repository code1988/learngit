...
struct proto_shell_handler {
	...
	struct proto_handler proto; // 记录了协议的公共部分信息，基类
	char *config_buf;           // 记录了字串形式的json参数"config"的值
	char *script_name;          // 记录了该shell脚本的相对路径名
	bool init_available;
	bool no_proto_task;
...
/* 将记录在指定json对象中的协议信息保存到struct proto_handler结构的实例中，并注册到全局的handler树中
 * @script  脚本相对路径名
 * @name    协议名(比如dhcp)
 * @obj     记录了该协议信息的json对象
 */
...
proto_shell_add_handler(const char *script, const char *name, json_object *obj)
{
    ...
    // 将该协议注册到全局的avl树handlers中
	add_proto_handler(proto);
}

// static/dhcp/pppoe等接入协议初始化
void proto_shell_init(void)
{
    // 打开"/lib/netif/"目录下的proto目录
	proto_fd = netifd_open_subdir("proto");
	if (proto_fd < 0)
		return;

    // 执行proto目录下的每个shell文件，并调用proto_shell_add_handler注册每种接入协议
