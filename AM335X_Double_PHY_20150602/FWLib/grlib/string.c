...
//*****************************************************************************
#include "font2424.h"
#define	BSPGUI_FFONT_X			24u				// 全角字x位数
#define	BSPGUI_FFONT_Y			24u				// 全角字y位数
#define	BSPGUI_FFONT_Y_A		1u				// 全角字显示时需要添加的空白列数
#define	BSPGUI_HFONT_X			24u				// 半角字x位数
#define	BSPGUI_HFONT_Y			12u				// 半角字y位数
#define	BSPGUI_HFONT_Y_A		0u				// 半角字显示时需要添加的空白列数
#define	__BSPGUI_USE_FONT		2				// 字库,0:不使用汉字库;1:使用一级字库;2:使用二级字库
//#define	GUI_FONT_DBC  _FONT_DBC		        // 半角字库
void GrStringDraw(const tContext *pContext, const char *pcString, int lLength,
             int lX, int lY, unsigned int bOpaque)
{
    int lIdx, lX0, lY0, lCount, lOff, lOn, lBit;
    const unsigned char *pucData;
    tContext sCon;
    static unsigned int ch0 = 0,ch1 = 0;
     //确定字符在列表中的位置
    ...
    //中文显示
    if(sCon.pFont->ucFormat == FONT_CH_STYLE )//添加的中文字库显示标示
    {
        unsigned char width,i,m,Colum;
        
        Colum=(BSPGUI_FFONT_X) / 8;
        while(*pcString&&lLength--)
        {
            ...
            for(i=0; i<width; i++)//描绘背景
            {
                GrLineDrawV(&sCon, lX + i, lY, lY + sCon.pFont->ucHeight);
            }              
            if(*pcString<0x80)
            {
                return;//半角字库由于初始化指针不好赋值，暂时不做。
            }
            else
            {
                ch0 = 0;
                ch1 = 0;
                // 计算偏移地址
                // 全角符号范围：0xA1A0 ~ 0xA9FF
                if((*(pcString+0)>=0xA1) && (*(pcString+0)<0xAA))
                {
                    // 得到对应字库存储区中的段首地址				
                    ch1 = *pcString++;					// 字库符号高字节
                    ch0 = *(pcString+0);					// 字库符号低字节
                    ch0 = ((ch1 - 0xA1) * 94 + ((ch0) - 0xA1));	// 每段字库有94个汉字（不是96），就是0xXXA1~0xXXFE                        
                    
                    CharIndex = ch0 * BSPGUI_FFONT_Y * ((BSPGUI_FFONT_X) / 8);  //获得汉字在字库位置                 
                } 
                // 汉字范围：0xB0A0 ~ 0xD7FF
#if __BSPGUI_USE_FONT == 1
                else if((*(pcString+0)>=0xB0) && (*(pcString+0)<0xD8))	// 一级字库
#else
                else if((*(pcString+0)>=0xB0) && (*(pcString+0)<0xF8))	// 二级字库
#endif	//__BSPGUI_USE_FONT == 1
                {
                    // 得到对应字库存储区中的段首地址
                    ch1 = *pcString++;					// 字库符号高字节
                    ch0 = *(pcString+0);					// 字库符号低字节
                    // 每段字库有94个汉字（不是96），就是0xXXA1~0xXXFE。加1，是为了除去0xXXA0;减6是去除0xAAA0~0xAFFF之间的空白
                    ch0 = ((ch1 - 0xA1 - 6) * 94 + ((ch0) - 0xA1));
                    CharIndex = ch0 * BSPGUI_FFONT_Y * ((BSPGUI_FFONT_X) / 8);
                }                    
            }
            for(m=0; m<Colum; m++)
            {
                for(i=0; i<width; i++)
                {
                    ...
                    while(lBit<9)
                    {
                        ...
                        if(sCon.pFont->pucData[CharIndex+i+m*width]&lIdx)
                        {
