...
typedef struct _FATFS {
    ...
    WORD    n_rootdir;        /* Number of root directory entries */				//根目录区目录项个数（只用于FAT12/16,FAT32不用）
    DWORD    winsect;        /* Current sector appearing in the win[] */			//当前缓冲区中存储的扇区号
    DWORD    sects_fat;        /* Sectors per fat */								//每个FAT所占扇区
    DWORD    max_clust;        /* Maximum cluster# + 1 */							//最大的簇数量
    DWORD    fatbase;        /* FAT start sector */									//FAT起始扇区
    DWORD    dirbase;        /* Root directory start sector (cluster# for FAT32) */	//根目录起始扇区
    DWORD    database;        /* Data start sector */								//数据起始扇区
#if !_FS_READONLY
    DWORD    last_clust;        /* Last allocated cluster */						//最后一个被分配的簇
    DWORD    free_clust;        /* Number of free clusters */						//空闲簇个数
#if _USE_FSINFO
    DWORD    fsi_sector;        /* fsinfo sector */									//存放文件系统信息的扇区（只用于FAT32）
    BYTE    fsi_flag;        /* fsinfo dirty flag (1:must be written back) */		//标记文件系统信息是否被改动，为1时要回写
    BYTE    pad2;
#endif
#endif
    BYTE    fs_type;        /* FAT sub type */										//FAT子类型，一般在mount时用，置0表示未挂载
    BYTE    sects_clust;    /* Sectors per cluster */								//每个簇的扇区数目（1，2，4...128）
#if S_MAX_SIZ > 512
    WORD    s_size;            /* Sector size */									//每个扇区字节数(只有扇区大于512字节时才用)
#endif
    BYTE    n_fats;            /* Number of FAT copies */							//文件分配表的数目，一般为2个
    BYTE    drive;            /* Physical drive number */							//物理驱动号，一般为0
    BYTE    winflag;        /* win[] dirty flag (1:must be written back) */			//标记文件是否被改动，为1时要回写
#ifdef SOC_CACHELINE_SIZE_MAX
#if (PAD1_SIZE != 0)
    BYTE    pad1[PAD1_SIZE];
#endif
    ...
    BYTE    pad1;
#endif
    BYTE    win[S_MAX_SIZ];    /* Disk access window for Directory/FAT */			//扇区缓存
}PACKED FATFS;


/* Directory object structure */
typedef struct _DIR {
    WORD    id;            /* Owner file system mount ID */							//文件系统挂载ID
    WORD    index;        /* Current index */										//当前读写索引号
    FATFS*    fs;            /* Pointer to the owner file system object */			//指向文件系统的指针
    DWORD    sclust;        /* Start cluster */										//文件其实簇
    DWORD    clust;        /* Current cluster */									//当前簇
    DWORD    sect;        /* Current sector */										//当前操作扇区
} PACKED DIR;
...
typedef struct _FIL {
    ...
    FATFS*    fs;                /* Pointer to the owner file system object */		//指向文件系统的指针
    DWORD    fptr;            /* File R/W pointer */								//文件当前读写指针位置，当文件打开时为0
    DWORD    fsize;            /* File size */										//文件大小
    DWORD    org_clust;        /* File start cluster */								//文件起始簇
    DWORD    curr_clust;        /* Current cluster */								//当前簇
    DWORD    curr_sect;        /* Current sector */									//当前操作扇区
#if _FS_READONLY == 0
    DWORD    dir_sect;        /* Sector containing the directory entry */			//包含目录项的扇区
    BYTE*    dir_ptr;        /* Ponter to the directory entry in the window */		//指向目录项的指针
#endif
#ifdef SOC_CACHELINE_SIZE_MAX
#if (PAD3_SIZE != 0)
    BYTE    pad3[PAD3_SIZE];
#endif
#endif
    BYTE    buffer[S_MAX_SIZ];    /* File R/W buffer */								//文件读写缓存
} PACKED FIL;


/* File status structure */
typedef struct _FILINFO {
    DWORD fsize;            /* Size */												//文件大小
    WORD fdate;             /* Date */												//最新修改日期
    WORD ftime;             /* Time */												//最新修改时间
    BYTE fattrib;           /* Attribute */											//文件属性
    char fname[8+1+3+1];    /* Name (8.3 format) */									//文件名
} PACKED FILINFO;
