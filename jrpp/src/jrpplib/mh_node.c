...
int node_parse_hello_packet(rppPort_t *this, RPP_PDU_T *pdu)
{
    ...
    else
    {
        ...
        if(ring->status != hello->ring_state)
        {
            ...
            // 节点通过hello报文从健康切换到破裂时，必须将fail定时器立即关掉，否则有风暴风险
            if(ring->status == RPP_FAULT)
            {
...
int node_parse_report_packet(rppPort_t *this, RPP_PDU_T *pdu)
{
    ...
    log_info("port %d recieve a packet for %s report",this->port_index,(report->type == MSG_ENDPOINT_TO_ENDPOINT)?"endpoint to endpoint":"linkdown");
    ring->hello_syn = 0;
    if(report->type == MSG_ENDPOINT_TO_ENDPOINT)
    {
        if(ring->status != RPP_HEALTH)
        {
            if(ring->node_role == NODE_TYPE_TRANSIT)
            {
                ...
                {
                    LOG_ERROR("peer endpoint recv endpointtoendpoint report packet");
                    return 0;
                }
...
int node_parse_cmd_packet(rppPort_t *this, RPP_PDU_T *pdu)
{
    ...
    {
        LOG_ERROR("get self node rsp ,abandono");
        return 0;
    }
    ...
    switch(cmd->type)
    {
        case CMD_GET_NODE_REQ:
            ring_build_cmd_rsp(ring,this,CMD_GET_NODE_RSP,pdu->hdr.src_mac);
            tx_cmd(this);

            if(!memcmp(pdu->hdr.src_mac,peer->neighber_mac,MAC_LEN))
                return 0;
            if(peer->link_st == LINK_UP)
                _node_fwd_cmd(peer,pdu);
            break;
        case CMD_GET_NODE_RSP:
            //log_info("get node rsp,dst mac[%02x:%02x:%02x:%02x:%02x:%02x]",\
...
void node_deal_link_down(rppRing_t *ring,rppPort_t *this)
{
    ...
    else
    {
         ...
         */
        if(this->neighber_vaild && peer->neighber_vaild)
            ballot_ittv_minus(peer);
    }
    
    if(peer->link_st == LINK_DOWN)
    {
        memcpy(this->endpoint,RPP_OUT_get_mac(),MAC_LEN);
        memcpy(peer->endpoint,RPP_OUT_get_mac(),MAC_LEN);
    }
}
